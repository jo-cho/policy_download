...
if __name__ == '__main__':
    ...
    for n in range(1,21):
        ...
        driver.find_element(By.CSS_SELECTOR,
            f'#ui_contents > div.page_conts-bar.policy_material > div#listArea > ul > li:nth-child({n}) > div > button').click()
        try:
            pdf_link = \
                [i.get_attribute("href") for i in driver.find_elements(By.CSS_SELECTOR,
                                                                       f'#ui_contents > div.page_conts-bar.policy_material > div#listArea > ul > li:nth-child({n}) > div.download_wrap > div.file_bubble > ul.con > li > a')][0]
            driver.get(pdf_link)
